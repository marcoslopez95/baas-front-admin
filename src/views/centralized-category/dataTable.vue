...
import { Store } from '@/stores/centralizedCategoryStore';
// import UploadVoucher from './UploadVoucher.vue';

const store = Store()
const helper = helperStore()
helper.url = '/api/configs/centralized-category'
...
<template>
  ...
  >
    <thead>
      <tr>
        ...
        <th class="text-uppercase">{{$t('commons.Description')}}</th>
  ...
  >
    <VCard>
      <VCardTitle>{{$t('commons.Update')}} {{$t('views.centralized-accounts.singular')}}</VCardTitle>
      <VCardText>
        <VRow>
          <VCol cols="12">
